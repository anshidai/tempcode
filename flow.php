...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    else
    {
        ...
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent, $goods->color_size, $goods->custom_attr))
        {
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'tel'           => empty($_POST['mobile'])        ? '' :   compile_str(make_semiangle(trim($_POST['mobile']))),
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if($cart_goods) {
        foreach($cart_goods as $k=>$val) {
            ...
            if(empty($val['custom_attr'])) {
                $cart_goods['goods_list'][$k]['goods_attr'] = str_replace('|||', ',', $val['goods_attr']);
                $cart_goods['goods_list'][$k]['goods_attr'] = ucwords($cart_goods['goods_list'][$k]['goods_attr']);    
            }
            else {
                if($val['goods_attr']) {
                    $colorsize_attr = explode('|||', $val['goods_attr']);
                    foreach($colorsize_attr as $k2=>$val2) {
                        $tms = explode(':', $val2);
                        $attr_color[$k2]['name'] = ucwords($tms[0]);
                        $attr_color[$k2]['value'] = $tms[1];         
                    }
                    $cart_goods[$k]['goods_attr'] = $attr_color;
                }
                ...
                foreach($custom_attr as $k3=>$val3) {
                    ...
                    if($tms[0] == 'height') {
                        ...
                        $custom[$k3]['custom_name'] = 'Waist Size';    
                    } else if($tms[0] == 'hip_size') {
                        $custom[$k3]['custom_name'] = 'Hip Size';    
...
else
{
    ...
    if($cart_goods['goods_list']) {
        foreach($cart_goods['goods_list'] as $k=>$val) {
            ...
            if(empty($val['custom_attr'])) {
                ...
                if($val['goods_attr']) {
                    ...
                    $cart_goods['goods_list'][$k]['goods_attr'] = $attr_color;
                }
