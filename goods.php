...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $goods['sale_num'] = count_price((1-$goods['shop_price']/$goods['market_price'])*100); 
        ...
        $commentlist = get_goods_comment($goods['goods_id']);
        $comment = array(
                'count' => $commentlist ?count($commentlist): '0', //全部
                'week_1' => get_comment_total($goods['goods_id'], 'week_1'), //1周前
                'week_4' => get_comment_total($goods['goods_id'], 'week_4'), //4周前
                'month_6' => get_comment_total($goods['goods_id'], 'month_6'), //6月前
                'star1' => 0, //1颗星
                'star2' => 0, //2颗星
                'star3' => 0, //3颗星
                'star4' => 0, //4颗星
        ...
        if($commentlist) {
            ...
            $comment['star1_per'] = round(($comment['star1']/($comment['count']*3))*100);
            $comment['star2_per'] = round(($comment['star2']/($comment['count']*3))*100);
            $comment['star3_per'] = round(($comment['star3']/($comment['count']*3))*100);
            $comment['star4_per'] = round(($comment['star4']/($comment['count']*3))*100);
            $comment['star5_per'] = round(($comment['star5']/($comment['count']*3))*100);
        }
        ...
        $catinfo = $db->getRow("SELECT cat_id,cat_name FROM ".$ecs->table('category'). " WHERE cat_id=".$goods['cat_id']." LIMIT 1");
        $catinfo['cat_name'] = str_replace('Dresses', 'Dress', $catinfo['cat_name']);
        $smarty->assign('catinfo', $catinfo); 
        $smarty->assign('sitename', $GLOBALS['_CFG']['shop_name']); 
        ...
        if($goods_attr_tmp) {
            ...
            $goods_attr['Silhouette']['name'] = 'Silhouette';            
            $goods_attr['Silhouette']['value'] = $goods_attr_tmp['silhouette'];
            
            $goods_attr['Hemline/Train']['name'] = 'Hemline/Train';            
            $goods_attr['Hemline/Train']['value'] = $goods_attr_tmp['hemline_train'];
            
            $goods_attr['Trend Collections']['name'] = 'Trend Collections';            
            $goods_attr['Trend Collections']['value'] = $goods_attr_tmp['trend_collections'];
            
            $goods_attr['Neckline']['name'] = 'Neckline';            
            $goods_attr['Neckline']['value'] = $goods_attr_tmp['neckline'];
            
            $goods_attr['Body Shape']['name'] = 'Body Shape';            
            $goods_attr['Body Shape']['value'] = $goods_attr_tmp['body_shape'];
            
            $goods_attr['Fabric']['name'] = 'Fabric';            
            $goods_attr['Fabric']['value'] = $goods_attr_tmp['fabric'];
            
            $goods_attr['Style']['name'] = 'Style';            
            $goods_attr['Style']['value'] = $goods_attr_tmp['style'];
            
            $goods_attr['Season']['name'] = 'Season';            
            $goods_attr['Season']['value'] = $goods_attr_tmp['wedding_venues'];
            
            $goods_attr['Sleeve Length']['name'] = 'Sleeve Length';            
            $goods_attr['Sleeve Length']['value'] = $goods_attr_tmp['sleeve_length'];
            
            $goods_attr['Sleeve Style']['name'] = 'Sleeve Style';            
            $goods_attr['Sleeve Style']['value'] = $goods_attr_tmp['sleeve_style'];
            
            $goods_attr['Embellishments']['name'] = 'Embellishments';            
            $goods_attr['Embellishments']['value'] = $goods_attr_tmp['embellishments'];
            
            $goods_attr['Back Details']['name'] = 'Back Details';            
            $goods_attr['Back Details']['value'] = $goods_attr_tmp['back_details'];
        ...
        if(empty($goods_attr)) {
            $goods_attr = $properties['pro']['Attribute'];    
        }
        $smarty->assign('goods_attr', $goods_attr); // 商品属性
        ...
        $seodescription = '';
        if($properties['pro']['Attribute']) {
            foreach($properties['pro']['Attribute'] as $v) {
                if($v['value']) {
                    $attribute_tmp[$v['name']] = $v['value'];   
                }
            } 
        ...
        if(isset($_GET['debug'])) {
            //var_dump($properties['pro']['Attribute']);
        }
...
function count_price($price)
{
    ...
    elseif($price >30 && $price <= 35) {
        $num = 35;                       
    }
    elseif($price >35 && $price <= 40) {
        $num = 40;                       
    }
    elseif($price >40 && $price <= 45) {
        $num = 45;                       
    }
    elseif($price >45 && $price <= 50) {
        $num = 50;                       
    }
    elseif($price >50 && $price <= 55) {
        $num = 55;                       
    }
    elseif($price >55 && $price <= 60) {
        $num = 60;                       
    }
    elseif($price >60 && $price <= 65) {
        $num = 65;                       
    }
    elseif($price >65 && $price <= 70) {
        $num = 70;                       
    }
    elseif($price >70 && $price <= 75) {
        $num = 75;                       
    }
    elseif($price >75 && $price <= 80) {
        $num = 80;                       
    }
    elseif($price >80 && $price <= 85) {
        $num = 85;                       
    }
    elseif($price >85 && $price <= 90) {
        $num = 90;                       
    }
    elseif($price >90 && $price <= 100) {
        $num = 95;                       
    }
    return $num;
}
...
function get_comment_total($goods_id, $date = 'all')
{
