...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    $smarty->assign('cat_attr',         get_attribute_value());
    ...
    //针对动态参数增加noindex
    if(isset($_GET['price_min']) || isset($_GET['price_max']) || isset($_GET['filter_attr']) || isset($_GET['sort']) || isset($_GET['order'])) {
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id,cat_describe FROM ' . $GLOBALS['ecs']->table('category')." WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.click_count, g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['click_count']      = $row['click_count']? $row['click_count']: 0;
        $arr[$row['goods_id']]['sale_num']         = count_price((1-$row['shop_price']/$row['market_price'])*100);
        
        $comment_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('comment') . " WHERE id_value='{$row['goods_id']}'");
        $arr[$row['goods_id']]['comment_count']     = $comment_count? $comment_count: 0;
    }
...
function get_attribute_value($cat_id = '2')
{
    $sql = "SELECT attr_id,cat_id,attr_name,attr_values
            FROM ".$GLOBALS['ecs']->table('attribute')." WHERE cat_id='{$cat_id}'";
    $res = $GLOBALS['db']->getAll($sql);
    if($res) {
        foreach($res as $k=>$v) {
            if($v['attr_values']) {
                $res[$k]['attr_values'] = explode("\n",$v['attr_values']);        
            }
        }
