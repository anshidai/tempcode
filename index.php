...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
   ...
   //缓存随机商品模块 begin
    ...
    $sql = "SELECT goods_id,goods_name,market_price,shop_price,goods_thumb FROM ".$GLOBALS['ecs']->table('goods')." B JOIN (SELECT CEIL(MAX(goods_id)*RAND()) AS ID FROM ".$GLOBALS['ecs']->table('goods').") AS m ON B.goods_id >= m.ID WHERE B.is_delete = 0 AND B.is_on_sale=1 LIMIT 5";
    $cachekey = md5($sql.'index');
    ...
    if(!$cachedata) {
        ...
        {
            $cachedata[$row_k['goods_id']]['name'] = $row_k['goods_name'];  
            $cachedata[$row_k['goods_id']]['market_price'] = price_format($row_k['market_price']);
            $cachedata[$row_k['goods_id']]['shop_price'] = price_format($row_k['shop_price']);
            $cachedata[$row_k['goods_id']]['thumb'] = get_image_path($row_k['goods_id'], $row_k['goods_thumb'], true);
            $cachedata[$row_k['goods_id']]['url'] = build_uri('goods', array('gid' => $row_k['goods_id']), $row_k['goods_name']);
        }
        $memcached->set($cachekey,serialize($cachedata), 3600*3);        
    }else {
        $cachedata = unserialize($cachedata);    
    }
    $smarty->assign('random_goods', $cachedata);
    //缓存随机商品模块 end
   
   //获取焦点图
   $smarty->assign('flashimg',     get_flash_xml());
