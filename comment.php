...
$act =  $_REQUEST['act'] ? htmlspecialchars(trim($_REQUEST['act'])) : '';
$goods_id = $_REQUEST['goods_id'] ? intval($_REQUEST['goods_id']) : 0;
$cmt_type = $_REQUEST['cmt_type'] ? intval($_REQUEST['cmt_type']) : 0;
$quslity_rank = $_REQUEST['quslity_rank'] ? intval($_REQUEST['quslity_rank']) : 0;
$value_rank = $_REQUEST['value_rank'] ? intval($_REQUEST['value_rank']) : 0;
$price_rank = $_REQUEST['price_rank'] ? intval($_REQUEST['price_rank']) : 0;
$title = $_REQUEST['title'] ? htmlspecialchars(trim($_REQUEST['title'])) : '';
$username = $_REQUEST['username'] ? htmlspecialchars(trim($_REQUEST['username'])) : '';
$user_id = empty($_SESSION['user_id']) ? 0 : $_SESSION['user_id'];
$email = empty($cmt->email) ? $_SESSION['email'] : trim($cmt->email);
$content = $_REQUEST['content'] ? htmlspecialchars(trim($_REQUEST['content'])) : ''; 
...
if($act == 'comment') {
    ...
    if(empty($goods_id)) {
        jsondata(101, 'Illegal request.');      
    } else if(empty($content)) {
        putjson(102, 'Please enter content.');
    } else if(strlen($content) < 10) {
        putjson(103, 'Please enter content may not be less than 20 characters.');
    }


    if(empty($username)) {
        if($_SESSION['user_id']) {
           $res = $db->getOne("SELECT user_name FROM ".$ecs->table('users')."WHERE user_id='{$_SESSION['user_id']}' LIMIT 1"); 
           $username = $res? $res: 'anonymous';
        }else {
            $username = 'anonymous';         
        }      
    }
    ...
    if(empty($title)) {
        $title = $db->getOne("SELECT goods_name FROM ".$ecs->table('goods')."WHERE goods_id='$goods_id' LIMIT 1");       
    }
    $data = array(
        ...
        'status' => $status,        
        'user_id' => $user_id,    
        'quslity_rank' => $quslity_rank,    
        'value_rank' => $value_rank,    
        'price_rank' => $price_rank,    
        'title' => $title,    
    );
    if(add_custom_comment($data)) {
        putjson(0, 'Comment on the success.');    
    } else {
        putjson(104, 'Comment on the failure.');
    }    
 ...
 * @return  void
 */
function add_custom_comment($data)
{

    /* 保存评论内容 */
    $sql = "INSERT INTO " .$GLOBALS['ecs']->table('comment') .
           "(comment_type, id_value, email, user_name, content, add_time, ip_address, status, user_id, quslity_rank, value_rank, price_rank, title) 
           VALUES ('{$data['comment_type']}', '{$data['id_value']}', '{$data['email']}', '{$data['user_name']}', '{$data['content']}', '{$data['add_time']}', 
           '{$data['ip_address']}', '{$data['status']}', '{$data['user_ip']}', '{$data['quslity_rank']}', '{$data['value_rank']}', '{$data['price_rank']}', '{$data['title']}')";
...
function putjson($stats = 100, $msg = 'error.')
{
    $json = array('status'=>$stats, 'msg'=> $msg);
