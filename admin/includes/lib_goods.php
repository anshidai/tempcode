...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('goods_attr_extend')." WHERE goods_id = '$goods_id' LIMIT 1";
    $attr_extend_info = $GLOBALS['db']->getRow($sql);
    
    $extend_zid = array(
        '12' => 'silhouette',
        '13' => 'hemline_train',
        '14' => 'wedding_venues',
        '15' => 'trend_collections',
        '16' => 'neckline',
        '17' => 'body_shape',
        '18' => 'fabric',
        '19' => 'style',
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $extend_name = $extend_zid[$val['attr_id']];
        $html .= '&nbsp;&nbsp;<input name="attr_extend['.$extend_name.']" value="'.$attr_extend_info[$extend_name].'" style="width:400px;" />';
        $html .= '</td></tr>';
    }
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral,source_url, " .
