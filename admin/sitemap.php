...
else
{
    ...
    $sm->build('../sitemap_sote_en_1.xml');

    
    /* 商品 */
    $sql = "SELECT goods_id, goods_name FROM " .$ecs->table('goods'). " WHERE is_delete = 0 AND is_on_sale=1";
    $res = $db->query($sql);
    ...
    $sm->build('../sitemap_goods_en_1.xml');
    
    
    $xmlstr = '<?xml version="1.0" encoding="utf-8"?>'."\n";
    $xmlstr .= '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n";
    $xmlstr .= '<sitemap>'."\n";
    $xmlstr .= '<loc>'.$ecs->url().'sitemap_sote_en_1.xml</loc>'."\n";
