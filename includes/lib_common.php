...
{
    //多国货币_修改过 开始 by ecshop模板开发中心
    if($GLOBALS['M'][$_SESSION['rate']][2]!='')
    {
     $price=$price / $GLOBALS['M'][$_SESSION['rate']][2];
    }
    //多国货币_修改过 结束 by ecshop模板开发中心
    ...
    return sprintf($GLOBALS['M'][$_SESSION['rate']][0].'%s '.$GLOBALS['M'][$_SESSION['rate']][ 1], $price);
    //多国货币_修改过 结束 by ecshop模板开发中心
}
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                if ($rewrite)
                {
                    $uri = 'Sort-'.urlencode(preg_replace('/[\.|\/|\?|&|\+|\\\|\'|"|,]+/', '', $append)).'-c'.$cid;
                    if (isset($bid))
                    {
                        $uri .= '-b' . $bid;
                    }
                    ...
                    if(isset($price_min) || isset($price_max) || isset($filter_attr) || !empty($sort) || !empty($order)) {
                        $uri .= '-Screening';   
                    }
                }
                else
        ...
        case 'goods':
            ...
            {
                $uri = $rewrite ? $gid : 'goods.php?id=' . $gid;
            }
    ...
    if ($rewrite)
    {
        ...
        {
            if($app == 'goods') {
                $uri = 'Goods-'.urlencode(preg_replace('/[\.|\/|\?|&|\+|\\\|\'|"|,]+/', '', $append)).'-g'.$gid;
            }
            elseif($app == 'category') {    
            }
            else {
