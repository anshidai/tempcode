...
{
    $sql = "SELECT custom_attr,rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
 ...
 * @param   integer $color_size 商品颜色大小
 * @param   integer $custom     自定义数据
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $color_size = array(), $custom = array())
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    ...
    //商品颜色、大小属性
    if(!empty($color_size)) {
        list($_color, $_size) = $color_size;
        $goods_attr         = "color:$_color|||size:$_size";
    }
    ...
    //用户自定义商品尺寸
    if(!empty($custom)) {
        $parent['custom_attr'] = "shoulder_width:{$custom[0]}|||";
        $parent['custom_attr'] .= "bust_size:{$custom[1]}|||";
